    ...
    if (source.__class__.__name__ not in ['Transform', 'ProgrammableFilter','Threshold','Text']):
     ...
     if (len(fileNames) > 1):
        ...
        if (source.__class__.__name__ in ['liggghts_Reader', 'liggghts_binReader']):
            SetDisplayProperties(source, Representation="3D Glyphs")
            DataRepresentation = Show(source)
            DataRepresentation.Scaling = 1
            DataRepresentation.ScaleMode = "Magnitude"
            DataRepresentation.ScaleFactor = 1
            DataRepresentation.GlyphType = "Sphere"
            DataRepresentation.GlyphType.Radius = 1
            DataRepresentation.SelectScaleArray = 'radius'
